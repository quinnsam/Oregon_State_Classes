...
// Crypto Varibles
static char *crypto_key = "group22key";
...
static void sbd_transfer(struct sbd_device *dev, sector_t sector,
    ...
    if (write){
        ...
        for (i = 0; i < nbytes; i += crypto_cipher_blocksize(crypt)) {
            crypto_cipher_decrpyt_one(crypt, dst + i, src + i);
        }
...
static void sbd_request(struct request_queue *q) {
    ...
    static int __init sbd_init(void) {
        ...
        crypt = crypto_alloc_cipher("blowfish", 0, 0);
        //tfm = crypto_alloc_tfm("blowfish", CRYPTO_TFM_MODE_ECB);
        /*
