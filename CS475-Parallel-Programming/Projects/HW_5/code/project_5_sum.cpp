...
#ifndef GLOB_WORK_GROUP
#define GLOB_WORK_GROUP 32
#endif


#ifndef LOC_WORK_GROUP
#define LOC_WORK_GROUP 32
#endif
...
main( int argc, char *argv[ ] )
{
	...
	{
		//hA[i] = hB[i] = hG[i] = (float) sqrt(  (double)i  );
		hA[i] = hB[i] = (float) sqrt(  (double)i  );
	}

	size_t dataSize = NUM_ELEMENTS * sizeof(float);
	size_t sumSize = NUM_WORK_GROUPS * sizeof(float);
    ...
    //cl_mem dG = clCreateBuffer( context, CL_MEM_READ_ONLY,  dataSize, NULL, &status );
	//if( status != CL_SUCCESS )
	//	fprintf( stderr, "clCreateBuffer failed (2)\n" );

	cl_mem dC = clCreateBuffer( context, CL_MEM_WRITE_ONLY, sumSize, NULL, &status );
	if( status != CL_SUCCESS )
    ...
    //status = clEnqueueWriteBuffer( cmdQueue, dG, CL_FALSE, 0, dataSize, hG, 0, NULL, NULL );
	//if( status != CL_SUCCESS )
	//	fprintf( stderr, "clEnqueueWriteBuffer failed (2)\n" );
	...
	if( status != CL_SUCCESS ) {
        ...
        PrintCLError(status, "clEnqueueNDRangeKernel", stderr);
    }
	...
	status = clEnqueueReadBuffer( cmdQueue, dC, CL_TRUE, 0, sumSize, hC, 0, NULL, NULL );
	if( status != CL_SUCCESS )
