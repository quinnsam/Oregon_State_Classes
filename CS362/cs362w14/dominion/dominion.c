...
int councilRoomEffect(int currentPlayer, int handPos, struct gameState *state) {
    ...
    for (i = 0; i < 4; i++)
		drawCard(currentPlayer, state); // +4 cards
	
	state->numBuys++; //+1 buy
	
	for (i = 0; i < state->numPlayers; i++) { //everybody else draws a card
		if ( i != currentPlayer ) {
			drawCard(i, state);
	    }
	}
...
int remodelEffect(int choice1, int choice2, int handPos, int currentPlayer, struct gameState *state) {
	int trashCard;
	int i;
...
int salvagerEffect(int choice1, int currentPlayer, int handPos, struct gameState *state) {
	//+1 buy
	state->numBuys++;
...
int stewardEffect(int choice1, int choice2, int choice3, int currentPlayer, int handPos, struct gameState *state) {
	if (choice1 == 1) {
		//+2 cards
		drawCard(currentPlayer, state);
		drawCard(currentPlayer, state);
