...
int feast_card(struct gameState *state, int currentPlayer, int choice1) {
	int i;
...
int remodel(struct gameState *state, int currentPlayer, int choice1, int choice2){
	int i, j;
	j = state->hand[currentPlayer][choice1];  //store card we will trash
...
int cardEffect(int card, int choice1, int choice2, int choice3,
	...
	switch (card) {
	case adventurer:
		if(adventurer_card(state, currentPlayer) == 0){
			return 0;
		}

	case council_room:
		if (council_room_card(currentPlayer, state, handPos) == 0) {
			return 0;
		}
	case feast:
		if (feast_card(state, currentPlayer, choice1) == 0) {
			return 0;
		}
	case gardens:
		return -1;
	...
	case mine:
		if(mine_card(state, currentPlayer, choice1, choice2, handPos) == 0){
			return 0;
		}

	case remodel:
		if( remodel_card(state, currentPlayer, choice1, choice2)){
			return 0;
		}
	case smithy:
		//+3 Cards
