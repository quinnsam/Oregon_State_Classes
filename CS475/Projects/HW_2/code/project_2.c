...
int main( int argc, char *argv[ ] ) {
	...
	for( int t = 0; t < NUMSTEPS; t++ )
	{
        // Coarse Grain
#ifdef COARS
        #pragma omp parallel for default(none), private(Bodies), schedule(static, 1)
#endif
#ifdef COARD
        #pragma omp parallel for default(none), private(Bodies), schedule(dynamic, 1)
#endif
        for( int i = 0; i < NUMBODIES; i++ )
		{
			float fx = 0.;
			...
			Body *bi = &Bodies[i];
#ifdef FINES
        #pragma omp parallel for default(none), shared(bi),private(i,Bodies), reduction(+:fx, fy, fz) 
#endif
#ifdef FINED
        #pragma omp parallel for default(none), shared(bi),private(i,Bodies), reduction(+:fx, fy, fz) 
#endif
