...
Diffie-Hellman Key Agreement:
    Cyclic groups, primitive roots:  in Z_n^*
        g  g^2 g^3 g^4 g^5 ...  Z_5^*
        1  1   1   1   1 
        2  4   3   1   2  
        3  4   2   1   3
        4  1   4   1   4

    Def: if {g, g^2, g^3, g^4, ...} = Z_n^*, then g is a primitiv root mod n.   {2,3 are prim roots mod 5}

    Ex: n = 8 (Z_8^* = {1,3,5,7})
        g  g^2 g^3 g^4 g^5 ...  Z_8^*
        1  1   1   1   1
        3  1   3   1   3
        5  1   5   1   5
        7  1   7   1   7            None are primitive roots.

    Obs: sequnece g, g^2, g^3, ... (mod n) is Cyclic: eventually hits 1 then repeats. (When gcd(g,n) == 1 <=> g in Z_n^*)

    Def: <g> = {g^i | i in Z_n^*} is cyclic group and g is its generator.

    Ex: Working mod 5, <2> = powers of 2
                           = Z_5^*
                           = <3> 
        (2,3 are different generators of same cyclic group)

    Ex: 17 is prim root mod 23 <=> everything in Z_23^* can be written as a power of 17 (mod 23)

Def: Given x in Z\_n^\*, prim root g in Z\_n^\* find a such that g^a =\_n x
        DISCREATE LOGARITHM PROBLEM (a = dlog\_g(x))
        ^- Belived to be hard (in appropriate choice of cyclic groups)
    Note: Given g, a: Easy to compute g^a in Z_n^* 

Diffie-Hellman:
    Protocol between Alice and Bob.
     * Alice and Bob have never spoken before, no shared secrents.
     * Establish a secret key that only they know by exchanging msgs on a public channel. 

    Parameters: (Public)
        1) Prime P
        2) prim root g mod p (<g> = Z_p^*)
...
Note: This description uses Z\_p^\*. as cyclic group but DH key agreement insecure with Z\_p^\*
    Better Choice: Group generated by g^2 where g prim root mod p and (p-1)/2 is prime. 

Def: "(t,K) <- execprotocol"
    means: run prot as Alice & Bob, let t denote transcript, K denote key.

Security Def: Key Agreement prot is secure if: 
    Query():
        (t,k) <- execprotocol
        return (t,k)
    ...
    idea: Can't tell "true" k from a totally indep. key, given t.

Secuity of DH: subtitute DHKA into above
    DHQuery():
        a,b <- Z_n                  This statement is equivalent to security of DHKA
        ret (g^a, g^b, g^ab)        Called Decisional Difie-Hellman assumption (DDH)
             \ t /      k
...
Public-Key encryption:
    * Keygen: output (pk,sk)
    * Enc: input is pk, plaintext -> output ctxt
    * Dec: input is sk, ctxt -> output ptxt
